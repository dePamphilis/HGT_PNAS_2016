...
# on the line that starts with "for tree in tree_files:"
...
# aegyptiaca genes start with OrAeBC5) strongly group with donor genes from Monocots. This is a stringent approach that requires two nodes that strongly support the grouping
...
if (len(sys.argv) != 2 ):
	print "get_HGT_from_monocots2parasites_strigent2nodes.py <newick_trees_dir>"
	exit()
...
##########################################################################################
        ...
        for row in evts:
            ...
            if (len(left) > 0 and len(right) > 0):
                ...
                if ( (mono_left >0 and mono_right >0) or (mono_left ==0 and mono_right >0) or (mono_left >0 and mono_right ==0) )  and (rosid_left ==0) and (rosid_right ==0) and (asterid_left ==0) and (asterid_right==0) and (other_left==0) and (other_right==0):
                                                    ...
                                                    if ances_dict[i] is None:
                                                        ...
                                                        if node in ancestor_1 or node == ancestor_1:
                                                            if node.support == max_value:
                                                                #print node
                                                                node2 = node
                                            print "the 2nd highly supported node consisting the parasite gene and the monocot donor is " + str(node2.support) + " with the node shown as:"
                                            print node2
